...
namespace BL
{
    ...
    {
        private const string apiKey = "FUWJN0RckiSo1rAQSbDQ~qBQt4TxaMDjDd2FIJyn42A~AigAXFnUBlTTheND7xl3LHVxr03YpammIUNKj-CZ_ntjT-uHAaVEisKWGMVnIUbQ";
        // private const string ServiceUri = "http://maps.googleapis.com/maps/api/geocode/xml?address={0}&region=be&sensor=false&key=AIzaSyAdFS6LDKmiZjFoQt3HYXc09BDfrUak5oU";
        ...
        private void ValidateGeocodeResponseStatus(string status, string address)
        {
            ...
            {
                case "ZERO_RESULTS":
                    string message = string.Format("No Coordinate found for address \"{0}\".", address);
                    throw new Exception(message);// new UnknownAddressException(message);
                case "OVER_QUERY_LIMIT":
                    throw new Exception("OVER_QUERY_LIMIT");// new OverQueryLimitException();
                case "OK":
                    break;
                default:
